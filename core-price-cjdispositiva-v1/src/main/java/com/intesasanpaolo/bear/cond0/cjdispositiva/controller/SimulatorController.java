...
import com.intesasanpaolo.bear.cond0.cjdispositiva.command.TestRollbackCommand;
...
@RestController
@RequestMapping("cjdispositivaSimulator")
@Validated
public class SimulatorController extends CoreController {
	...
	@PostMapping(value = "/rollbackTest", produces = "application/json")
	public ResponseEntity<String> inserimento() throws Exception
			 {
		...
		TestRollbackCommand testRollbackCommand = beanFactory
				.getBean(TestRollbackCommand.class);

		EsitoResponseResource esito = testRollbackCommand.execute();
