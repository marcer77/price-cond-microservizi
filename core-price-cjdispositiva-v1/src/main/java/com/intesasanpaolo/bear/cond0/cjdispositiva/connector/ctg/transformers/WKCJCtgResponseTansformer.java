...
import com.intesasanpaolo.bear.cond0.cjdispositiva.model.ctg.OutRAF;
import com.intesasanpaolo.bear.cond0.cjdispositiva.model.ctg.OutRAP;
import com.intesasanpaolo.bear.cond0.cjdispositiva.model.ctg.OutSTP;
...
public class WKCJCtgResponseTansformer implements ICtgResponseTransformer<C_WKCJS00, WKCJResponse> {
	...
	public WKCJResponse transform(CtgConnectorResponse<C_WKCJS00> ctgResponse) {
		...
		OutEsi outEsiModel = OutEsi.builder().mdwEsiRetc(outEsi.MDW_ESI_RETC).mdwEsiMsg(outEsi.MDW_ESI_MSG)
				.mdwEsiAnom(outEsi.MDW_ESI_ANOM).build();
		OutSeg outSegModel = OutSeg.builder().txtSegnalazione(outSeg.TXT_SEGNALAZIONE)
				.livelloSegnalazione(outSeg.LIVELLO_SEGNALAZIONE).build();
...
/*
		...
		if (hasSomething(outBody.OUTRAF)) {
			Arrays.asList(outBody.OUTRAF).forEach(out -> {
				OutRAF outRAF = OutRAF.builder().codDds(out.COD_DDS).codDip(out.COD_DIP).codPar(out.COD_PAR)
						.codSet(out.COD_SET).numRap(out.NUM_RAP).prgInv(out.PRG_INV).build();
		...
		if (hasSomething(outBody.OUTRAP)) {
			Arrays.asList(outBody.OUTRAP).forEach(out -> {
				OutRAP outRAP = OutRAP.builder().dds(out.DDS).prgInv(out.PRG_INV).build();
			...
			Arrays.asList(outBody.OUTSTP).forEach(out -> {
				...
				if (hasSomething(out.OUTRC4)) {
					Arrays.asList(out.OUTRC4).forEach(tmp -> {
						OutRC4 outRC4 = OutRC4.builder().campi41(tmp.CAMPI4_1).campi42(tmp.CAMPI4_2)
								.campi43(tmp.CAMPI4_3).campi44(tmp.CAMPI4_4).build();
						outRC4List.add(outRC4);
					});
				}
				if (hasSomething(out.OUTRCZ)) {
					Arrays.asList(out.OUTRCZ).forEach(tmp -> {
						OutRCZ outRCZ = OutRCZ.builder().dataDeco(tmp.DATA_DECO).descrCdz(tmp.DESCR_CDZ)
								.evidDescr(tmp.EVID_DESCR).evidDtdec(tmp.EVID_DTDEC).evidNota(tmp.EVID_NOTA)
								.evidVal(tmp.EVID_VAL).indNota(tmp.IND_NOTA).valore(tmp.VALORE).build();
				...
				if (hasSomething(out.OUTRNO)) {
					Arrays.asList(out.OUTRNO).forEach(tmp -> {
						OutRNO outRNO = OutRNO.builder().evidNrNota(tmp.EVID_NR_NOTA).evidTxNota(tmp.EVID_TX_NOTA)
								.numNota(tmp.NUM_NOTA).txNota(tmp.TX_NOTA).build();
				...
				if (hasSomething(out.OUTRPR)) {
					Arrays.asList(out.OUTRPR).forEach(tmp -> {
						OutRPR outRPR = OutRPR.builder().testoProm(tmp.TESTO_PROM).build();
						outRPRList.add(outRPR);
					});
				}
				if (hasSomething(out.OUTRTS)) {
					Arrays.asList(out.OUTRTS).forEach(tmp -> {
						OutRTS outRTS = OutRTS.builder().evidTesto(tmp.EVID_TESTO).testo(tmp.TESTO).build();

						outRTSList.add(outRTS);
					});
				}

				OutSTP outSTP = OutSTP.builder().flContinua(out.FL_CONTINUA).keyPrecontr(out.KEY_PRECONTR)
						.progrStp(out.PROGR_STP).progStrut(out.PROG_STRUT).tipoSez(out.TIPO_SEZ)
						.tipoStrut(out.TIPO_STRUT).outRC4List(outRC4List).outRNOList(outRNOList).outRCZList(outRCZList)
						.outRPRList(outRPRList).outRTSList(outRTSList).build();
