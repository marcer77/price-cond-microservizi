...
//	@MockBean
	...
	@MockBean
	private WSGestioneCommand wsGestioneCommand;

	private EsitoOperazioneCJPOSV2 esito;
	
	private NewAccountOutput newAccountOutput;
	...
	public void initMocks() throws Exception {
		...
		proposteCJPOSWSRevocaPropostaCommand = new ProposteCJPOSWSRevocaPropostaCommand();
	}
...
//	@Test
	public void wsGestioneCommandTest() throws Exception {
		
		log.debug(" wsGestioneCommand START ");

		String URI = "/simulator/wsGestione";

		Mockito.when(wsGestioneCommand.execute()).thenReturn(newAccountOutput);

		mockMvc.perform(MockMvcRequestBuilders.post(URI).content(asJsonString(header)).contentType(MediaTypes.HAL_JSON))
				.andExpect(status().isOk());
		
		log.debug(" wsGestioneCommand END ");
	}
