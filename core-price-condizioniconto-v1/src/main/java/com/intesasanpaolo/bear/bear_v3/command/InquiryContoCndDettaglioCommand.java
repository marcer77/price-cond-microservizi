...
public class InquiryContoCndDettaglioCommand extends BaseCommand<InquiryContoCndDettaglioOutput> {
	...
	public InquiryContoCndDettaglioOutput doExecute() throws Exception {
		...
		List<CondizioneContoDettaglio> condizioniStd = new ArrayList<>(); // una singola copia della lista di condizioni
																			// - per recuperare il val STD
		List<CondizioneContoDettaglio> condizioniOut = new ArrayList<>(); // intera lista di condizioni - eventualmente
																			// ripetute per tipo livello
		...
		/*
		 * String prod = request.getCdProdotto(); List<DriverInq> driver =
		 * request.getDriver();
		 * 
		 * String conv = request.getCdConv(); String rapp = request.getCdRapporto();
		 * 
		...
		for (CondizioneContoDettaglio cond : condizioniStd) {
			...
			if (condizioneDeroga != null)
				if (condizioneDeroga.getCD_UDM().equals("3"))
					...
					condizioneDeroga.setValNumericoStandard(jdbcService.getValSTD(condizioneDeroga.getCD_CONDIZIONE(),
							cdIstituto, request.getDtOperazione()));
		...
		if (req.size() > 0) {
			...
			out.addAll(service.inquiryContoCnd(iibCdprcmsRequest)); // PROBLEMA ULTIMO CASO DI TESTO
			logger.info("response:" + new Gson().toJson(out));
		}

		if (condizioniStd.isEmpty() || (out.size() > 0 && out.get(0).getNbpErrorInfo() != null
				&& out.get(0).getNbpErrorInfo().size() > 0)) { // ERR
			output.setCdEsito(CondizioniContoUtils.ESITO_KO);
			if (!CollectionUtils.isEmpty(out.get(0).getNbpErrorInfo())) // AGGIUNTO CONTROLLO NULLPOINTER
	...
	private List<CondizioneContoDettaglio> buildResponseFromCNDPRICEMS(List<IIBCDPRCMSSingleResponseType> response,
	...
	private BigDecimal calcolaValorePromo(CondizioneConto cond, BigDecimal prezzoRif,
		...
		if (cond.getCdTipoLivello().equalsIgnoreCase("P")) { // VA CALCOLATO A PARTIRE DAL VALORE, IN BASE AL TIPO PROMO
			...
			if (valCalc != null && valCalc.compareTo(BigDecimal.ZERO) < 0) // CORREZIONE NULLPOINTER
